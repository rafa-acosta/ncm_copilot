...
| `evaluate_control(control, device_config, golden_config) -> ControlResult` | `control`: one control dict from `controls.yaml`. `device_config`/`golden_config`: raw config text (not file paths). | A `ControlResult`. | None (pure). Dispatches by `control["control_id"]` to a `_check_control_XXXXX` method via `getattr`; if `control["manual_review"]` is true, skips straight to `MANUAL_REVIEW` status without calling any checker. |
...
Module-level constants also defined here: `STATUS_PASS`, `STATUS_FAIL`, `STATUS_EXCEPTION`, `STATUS_MANUAL_REVIEW`, `PRIORITY_CONTROL_IDS` (the set `{control_00001..00006, 00008..00015}`, reused by `main.py --priority-only` and `golden_config_builder.py --priority-only` and imported directly by `remediation_advisor`/tests).
...
| `render_control(control_id) -> RenderedControl \| None` | A control ID. | `None` if the ID isn't in `controls.yaml` (e.g. `control_00007`, or any ID not present at all). Otherwise a `RenderedControl`. | Appends to `self.missing`. Dispatches to one of three paths: (1) `control_00008` and any control listed in `_SUBSUMED_BY` render a one-line pointer comment instead of commands (currently just `control_00008`, whose content is fully covered by `control_00014`'s VTY block — rendering both independently would duplicate the `line vty` stanza); (2) `control_00012`/`control_00016`/`control_00018` (manual-review or policy-checklist-only, hardcoded by ID regardless of what `controls.yaml` carries) render `config_example` verbatim inside a `! MANUAL REVIEW REQUIRED` comment block; (3) everything else goes through `_render_tacacs` (only `control_00004`) or `_render_generic`. |
...
| `build(priority_only=False) -> str` | Optional filter flag. | The full rendered config text. | Resets and repopulates `self.missing`. Iterates `self.order` (optionally filtered to `compliance_engine.PRIORITY_CONTROL_IDS`), calling `render_control` for each and concatenating, with a generated header (device hostname pulled from `device_vars["control_00001"]["hostname"]` if present, plus a timestamp). |
| `save(output_path="golden_config.txt", priority_only=False) -> str` | Output path, optional filter flag. | The rendered text (also returned by `build`). | **Writes `output_path`** (creating parent directories if needed). |
