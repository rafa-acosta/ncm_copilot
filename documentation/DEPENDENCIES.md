...
| `click` | All four CLI entrypoints' argument parsing (`@click.command()` / `@click.option()`), plus `click.testing.CliRunner` in the test suite for CLI-level tests without spawning a real subprocess. | `main.py`, `golden_config_main.py`, `agent_assisted_coding_advise.py`, all `tests/test_*` files touching a CLI |
