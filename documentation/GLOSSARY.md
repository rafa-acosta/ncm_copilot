...
**Control / `control_id`** — One security requirement (e.g. "SNMP must use v3 with encryption"), identified by an ID like `control_00011`. IDs are 5-digit, zero-padded, and run `control_00001`–`control_00018` — all 18 are defined as entries in `controls.yaml` (control_00007, once a gap in the original source document, was filled in by a later spec revision — see `ARCHITECTURE_OVERVIEW.md`).
...
**Active controls / `ACTIVE_CONTROL_IDS`** — Controls `00001`–`00015` (15 total, contiguous) — the ones evaluated, scored, and rendered by default in this version, as opposed to `00016`–`00018` (broader policy-checklist categories, defined in `controls.yaml`/`render_order.yaml` for future use but never part of the active flow). The set is defined once as `compliance_engine.ACTIVE_CONTROL_IDS` and applied unconditionally by both Tool 1 (`main.py`) and Tool 2 (`golden_config_builder.py`'s `build()`) — there's no flag to opt in or out of this; it's the only mode.

**`deterministic_validation` vs. `manual_review`** — Two boolean fields on each control. `deterministic_validation: true` means a script can check it automatically. `manual_review: true` (currently only `control_00012`, Banners) flags that the content genuinely needs human judgment somewhere in the pipeline (e.g. whether banner wording matches corporate legal policy) — Tool 2 always renders these as a flagged placeholder rather than real commands, regardless of any automated check. Tool 1 only falls back to status `MANUAL_REVIEW` when a control has `manual_review: true` **and** no `_check_control_XXXXX` method exists; a control with both (control_00012's case) gets Tool 1's automated check instead — see `compliance_engine.py`'s module docstring. `control_00012`'s check is presence-only (does `banner motd` exist at all), not a wording/content review.
