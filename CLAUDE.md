...
- See `COMPLIANCE_ANALYZER_PROMPT.md` for the third component, the Compliance Remediation Advisor, which turns this tool's `--formats json` report output into an LLM-assisted remediation briefing. See `LOCAL_LLM_SETUP_PROMPT.md` for the local model backend it talks to. `agent_assisted_coding_advise.py` supports both a single device (`--report`) and batch mode (`--reports-dir`, same directory-discovery convention as Tool 6) - batch mode selects the LLM backend once for the whole run (see `llm_client.select_backend`'s `device_count` parameter) and skips (not aborts on) any one device whose report fails to load or brief.
