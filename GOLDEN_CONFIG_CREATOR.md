...
- **`device_vars.json` uses `controls.yaml`'s actual variable names**, not this spec's illustrative example, where they differ — per section 11's own instruction that `controls.yaml` is the naming source of truth.
...
- **Golden Config Creator realignment to `support_files/NCM Configuration Script.txt` (2026-09-15).** Several templates had drifted from the real production reference: control_00004 hardcoded `key 6` (the real script uses `key 0`); control_00008 rendered a single `permit ip any any` placeholder instead of the real 21-line Cloudflare-range-deny ACL; control_00014 rendered one combined `line vty <start> <end>` block via `exit` terminators, with no `line aux 0` and no `exec prompt timestamp`, instead of the real two-range (`0 4`/`5 15`) structure with `!` terminators; control_00005 rendered an extra `enable secret` line that belongs to control_00015 instead; control_00006 re-rendered `hostname`/`ip domain name` lines that belong to control_00001/00002. Fixed by: `key 0` literal; two new special-case renderers, `GoldenConfigBuilder._render_acl` and `_render_vty`, mirroring `_render_tacacs`'s existing pattern (a real Jinja `{% for %}` loop over a device_vars list - `acl_rules` and `vty_ranges` respectively - with the same missing-value-tracking and blank-line-collapse behavior); removing the misplaced lines from 00005/00006's templates; and reordering `render_order.yaml`'s 00003/00004/00005/00015 sequence to match the script's actual order (`00005, 00015, 00004, 00003`). `golden_config.txt`'s real command content now matches the script exactly (verified by `tests/test_golden_config_build_full.py::test_full_build_matches_real_ncm_configuration_script`, a line-by-line comparison ignoring `!` comments and blank lines) - cosmetic details (the script's decorative `! ====` divider comment, its exact blank-line spacing) were deliberately not reproduced, since neither Cisco nor this project's `ConfigTree` parser treat those as meaningful.
- **`golden_config.txt` (this tool's own default output, at the repo root) is now the Compliance Checker's source of truth for the real fleet audit**, superseding the previously hand-maintained `golden_config_11_08_2026.txt` (left in the repo, unreferenced, not deleted). See `documentation/CONTROL_UPDATE_REPORT.md`'s closing note.
